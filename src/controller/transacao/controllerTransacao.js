...
const inserirTransacao = async function (transacao, contentType) {
  try {
    ...
    if (
      !transacao.user_id ||
      !transacao.valor ||
      transacao.is_entrada === undefined ||
 ...
 * ATUALIZAR TRANSAÇÃO
 */
const atualizarTransacao = async function (
  codigo,
  transacao,
  contentType,
  user_id
  ...
  try {
    ...
    const transacaoExistente = await transacaoDAO.selectByCodigoTransacao(
      codigo
    );
    ...
    if (transacaoExistente.user_id !== user_id) {
      return {
        status: false,
        status_code: 403,
    ...
    const transacaoAtualizada = await transacaoDAO.updateTransacao(
      codigo,
      transacao
    );
 ...
 * EXCLUIR TRANSAÇÃO
 */
const excluirTransacao = async function (codigo, user_id) {
  try {
    if (!codigo || !user_id) {
      return MESSAGE.ERROR_REQUIRED_FIELDS;
    }
 ...
 */
const listarTransacoesPorUsuario = async function (user_id, filters = {}) {
  try {
    if (!user_id) {
      return MESSAGE.ERROR_REQUIRED_FIELDS;
    }

    const transacoes = await transacaoDAO.selectAllTransacoesByUser(
      user_id,
      filters
    );
...
const listarDespesas = async function (user_id, filters = {}) {
  try {
    ...
    const despesas = await transacaoDAO.selectDespesasTransacao(
      user_id,
      filters
    );
...
const listarEntradas = async function (user_id, filters = {}) {
  try {
    ...
    const entradas = await transacaoDAO.selectEntradasTransacao(
      user_id,
      filters
    );
